		...
		try {
				...
				for (ComEcfVenda venda : z.getComEcfVendas()) {
					if (venda.getComEcfVendaFechada() && !venda.getComEcfVendaCancelada()) {
						for (ComEcfVendaProduto eProd : venda.getComEcfVendaProdutos()) {
							if (!eProd.getComEcfVendaProdutoCancelado() && !embalagens.contains(eProd.getProdEmbalagem().getProdEmbalagemId())) {
								out.write(getDados(eProd.getProdEmbalagem()));
								out.flush();
								embalagens.add(eProd.getProdEmbalagem().getProdEmbalagemId());
							}
						}
					}
				}
			...
			if (estoque != null) {
				for (ProdProduto prod : estoque) {
					if (prod.getProdComposicoes().isEmpty() && !embalagens.contains(prod.getProdEmbalagem().getProdEmbalagemId())) {
