...
public class FormularioSped extends AFormulario<FisSped> {
	...
	public FormularioSped(FisSped classe, SisFuncao funcao) {
		super(classe, funcao);
		inicializar();
	}
	...
	public boolean setDados() {
		classe.setFisSpedTipo(cmbSped.getValue());
		classe.setFisSpedAno(Integer.valueOf(cmbAno.getValue()));
		classe.setFisSpedMes(Integer.valueOf(cmbMes.getValue()));
	...
	public void mostrarDados() {
		if (classe.getFisSpedAno() > 0) {
			chkFinalidade.setChecked(true);
			cmbSped.setValue(classe.getFisSpedTipo());
			cmbAno.setValue(classe.getFisSpedAno() + "");
			cmbMes.setValue(classe.getFisSpedMes() > 9 ? classe.getFisSpedMes() + "" : "0" + classe.getFisSpedMes());
