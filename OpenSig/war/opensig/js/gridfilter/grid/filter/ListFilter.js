	...
	init: function(){
		...
		if(this.store){
			...
			this.store = new Ext.data.Store({
				reader: new Ext.data.ArrayReader({id: 0}, [this.labelValue, this.labelField])
			});
	...
	onLoad: function(store, records){
		...
		for(var i=0, len=records.length; i<len; i++){
			var item = new Ext.menu.CheckItem({
				...
				group:   gid, 
				checked: this.value.indexOf(records[i].get(this.labelValue)) > -1,
				hideOnClick: false});
			
			item.itemId = records[i].get(this.labelValue);
