...
public class ComandoReBuy extends ComandoAcao {
	...
	public ComandoReBuy() {
		...
		AComando cmdFim = new AComando() {
			public void execute(Map contexto) {
				...
				for (Record rec : recs) {
					...
					int rebyes = rec.getAsInteger("pokerParticipanteReentrada") + 1;
					rec.set("pokerParticipanteReentrada", rebyes);
				}
			}
		};
		// executa o comando e atualiza todos relacionados
		ComandoExecutar cmdReBuy = new ComandoExecutar(cmdFim) {
			public void execute(Map contexto) {
				MessageBox.wait(OpenSigCore.i18n.txtRebuy(), OpenSigCore.i18n.txtAguarde());
				...
				Sql sqlForma = new Sql(new PokerParticipante(), EComando.ATUALIZAR, gf, pf);
				sqls.add(sqlForma);
		...
		// validando addon
		validaAddOn = new AComando(cmdReBuy) {
